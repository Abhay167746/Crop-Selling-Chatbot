...
export default function Navbar() {
  ...
  // Hide entirely on chatbot page — chatbot has its own header
  ...
  return (
    <>
      <style>{`
        ...
        .nav-lnk:hover { color: #7ecb5a; }
        .nav-lnk:hover::after { width: 100%; }
        .nav-lnk.active { color: #7ecb5a; }
      ...
      <motion.nav
        ...
        style={{
          ...
          background: scrolled ? "rgba(15,26,12,0.95)" : "rgba(15,26,12,0.7)",
          borderBottom: "1px solid rgba(42,58,36,0.6)",
      ...
      >
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 20px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div onClick={() => navigate("/")} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
            <img src={collegeLogo} alt="AgroAI" style={{ width: 32, height: 32, borderRadius: "50%", objectFit: "cover", border: "1.5px solid rgba(126,203,90,0.35)" }} />
            <span style={{ fontFamily: "'Noto Sans', sans-serif", fontSize: 17, fontWeight: 700, color: "#7ecb5a" }}>AgroAI</span>
          </div>

          {/* Desktop links */}
          <div style={{ display: "flex", alignItems: "center", gap: 24 }} className="desktop-nav">
            {links.map(l => (
              <NavLink key={l.to} to={l.to} end={l.to === "/"} className={({ isActive }) => `nav-lnk${isActive ? " active" : ""}`}>
                {l.label}
              </NavLink>
            ))}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }} className="desktop-nav">
            {!isLoggedIn ? (
              <>
                <NavLink to="/login" className="nav-lnk" style={{ padding: "6px 14px" }}>Login</NavLink>
                <NavLink to="/signup" style={{
                  background: "#2d7a22", color: "#fff",
              ...
              <button onClick={handleLogout} style={{
                ...
                color: "rgba(252,165,165,0.9)", padding: "6px 14px",
          ...
          >
            {[0, 1, 2].map(i => (
              <motion.span key={i}
                animate={i === 0 ? { rotate: menuOpen ? 45 : 0, y: menuOpen ? 7 : 0 } : i === 1 ? { opacity: menuOpen ? 0 : 1 } : { rotate: menuOpen ? -45 : 0, y: menuOpen ? -7 : 0 }}
                style={{ display: "block", width: 20, height: 1.5, background: "#7ecb5a", transformOrigin: "center" }}
              />
            ))}
          </button>
        </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              style={{ background: "rgba(15,26,12,0.98)", borderTop: "1px solid #1a2a14", overflow: "hidden" }}
            >
              <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
                {links.map(l => (
                  <NavLink key={l.to} to={l.to} end={l.to === "/"} className={({ isActive }) => `nav-lnk${isActive ? " active" : ""}`} style={{ fontSize: 15 }}>
                    {l.label}
                  </NavLink>
                ))}
                <div style={{ height: 1, background: "#1a2a14" }} />
                {!isLoggedIn ? (
                  <>
                    <NavLink to="/login" style={{ textAlign: "center", padding: "10px", border: "1px solid #2a3a24", borderRadius: 8, color: "#a8c89a", textDecoration: "none", fontSize: 14 }}>
                      Login
                    </NavLink>
                    <NavLink to="/signup" style={{ textAlign: "center", padding: "10px", background: "#2d7a22", borderRadius: 8, color: "#fff", textDecoration: "none", fontSize: 14, fontWeight: 700 }}>
                      🌾 Get Started
                    </NavLink>
                  </>
                ) : (
                  <button onClick={handleLogout} style={{ padding: "10px", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, color: "rgba(252,165,165,0.9)", background: "transparent", fontSize: 14, cursor: "pointer" }}>
                    Logout
                  </button>
                )}
              </div>
      ...
      <style>{`
        @media (min-width: 640px) { .hamburger { display: none !important; } }
